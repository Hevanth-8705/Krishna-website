...
import { Eye, Upload, Camera, Image as ImageIcon, Sparkles, Search, Bug, BookOpen, Compass, FileText, GitCompare, X, Loader2, Copy, Check, History, AlertTriangle, Plus } from 'lucide-react';
...
export default function KrishnaVision() {
  ...
  const [images, setImages] = useState<Array<{ preview: string; name: string; size: string; type: string }>>([]);
  ...
  const processFile = useCallback((file: File) => {
    ...
    reader.onload = (e) => {
      ...
      setImages((prev) => {
        // If not compare mode, replace. If compare mode, support up to 2 images.
        if (selectedMode !== 'COMPARE') {
          return [newImg];
        }
        return prev.length >= 2 ? [prev[0], newImg] : [...prev, newImg];
      });
    };
    reader.onerror = () => setVisionError('Failed to read the image file.');
    reader.readAsDataURL(file);
  }, [selectedMode, setVisionError]);
  ...
  const handleAnalyze = async () => {
    if (images.length === 0) {
      setVisionError('Please upload at least one image to analyze.');
      return;
    }
    ...
    try {
      ...
      if (auth.currentUser) {
        try {
          ...
          console.warn('[Krishna Vision] ID token retrieval warning:', authErr);
        }
      }

      const requestBody = {
        imageBase64: images[0].preview,
      ...
      if (contentType.includes('application/json')) {
        try {
          ...
          throw new Error('Krishna Vision received an invalid JSON response from server.');
        }
      } else {
        const text = await res.text();
        if (!text) {
          throw new Error('Krishna Vision server returned an empty response.');
        }
        throw new Error(`Server returned non-JSON response (${res.status}).`);
      }

      if (!res.ok || !data.success) {
        const normErr = normalizeApiError(data, `Krishna Vision analysis failed (${res.status}).`);
        throw new Error(normErr.message);
      }

      const resultPayload = data.data || data;

      const analysis: VisionAnalysis = {
        imagePreview: images[0].preview,
        analysis: resultPayload.analysis || 'Analysis completed.',
        mode: resultPayload.mode || selectedMode,
        model: resultPayload.model || 'Vision AI',
        confidence: resultPayload.confidence || 'OBSERVED',
      ...
      const normErr = normalizeApiError(err, 'Krishna Vision encountered an error. Please try again.');
      setVisionError(normErr.message);
    } finally {
        ...
        <div className="space-y-4">
          ...
          >
            {images.length === 0 ? (
              ...
              >
                ...
                <div className="text-center">
                  <p className="text-white font-medium">
                    {selectedMode === 'COMPARE' ? 'Drop 1 or 2 images here to compare' : 'Drop an image here or click to browse'}
                  </p>
                  <p className="text-xs text-gray-500 mt-1 font-mono">PNG, JPEG, WebP, GIF • Max 10MB each</p>
              ...
              <div className="glass-panel p-4 relative space-y-3">
                <div className="flex items-center justify-between border-b border-white/10 pb-2">
                  <span className="text-xs font-mono text-gray-400 uppercase tracking-wider">
                    {images.length === 1 ? 'Selected Image' : `Selected Images (${images.length})`}
                  </span>
                  <button
                ...
                <div className={cn("grid gap-3", images.length > 1 ? "grid-cols-2" : "grid-cols-1")}>
                  {images.map((img, idx) => (
                    <div key={idx} className="relative group rounded-xl bg-black/40 border border-white/10 overflow-hidden p-2">
                      <button
                        onClick={() => removeImageAt(idx)}
                        className="absolute top-3 right-3 z-10 w-7 h-7 rounded-full bg-black/70 border border-white/20 flex items-center justify-center hover:bg-red-500/40 hover:border-red-500 transition-colors"
                      >
                      ...
                      <img
                        src={img.preview}
                        alt={`Preview ${idx + 1}`}
                        className="w-full max-h-[260px] object-contain rounded-lg bg-black/20"
                      />
                      <div className="flex items-center justify-between mt-2 px-1 text-[11px] text-gray-400 font-mono">
                        <span className="truncate max-w-[140px]">{img.name}</span>
                        <span className="px-1.5 py-0.5 rounded bg-white/5 border border-white/10">{img.size}</span>
                      </div>
                    </div>
                  ))}
                </div>

                {selectedMode === 'COMPARE' && images.length < 2 && (
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="w-full py-2.5 rounded-xl border border-dashed border-[#60A5FA]/40 bg-[#60A5FA]/5 text-[#60A5FA] text-xs font-mono hover:bg-[#60A5FA]/10 flex items-center justify-center gap-2 transition-colors mt-2"
                  >
                    <Plus className="w-4 h-4" /> Add Second Image to Compare
                  </button>
                )}
              </div>
            )}

            <input ref={fileInputRef} type="file" accept="image/*" multiple={selectedMode === 'COMPARE'} onChange={handleFileSelect} className="hidden" />
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <button
                      onClick={handleAnalyze}
                      disabled={images.length === 0 || isAnalyzingVision}
                      className="mt-3 px-3 py-1.5 rounded-lg bg-[#FF3B3B]/10 border border-[#FF3B3B]/30 text-[#FF3B3B] text-xs font-semibold hover:bg-[#FF3B3B]/20 transition-colors flex items-center gap-1.5 disabled:opacity-50"
                    >
