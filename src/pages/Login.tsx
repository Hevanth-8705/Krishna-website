...
import { FirebaseHealthCheckPanel } from '../components/FirebaseHealthCheck';
...
export default function Login() {
  ...
  const [showFirebaseHealth, setShowFirebaseHealth] = useState(false);
  ...
  const handleSendResetEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResetStatus(`Reset failed: ${getFirebaseAuthErrorMessage(err, 'Failed to send reset link.')}`);
    } finally {
      ...
      <div className="w-full max-w-md z-10 space-y-6">
        ...
        {user ? (
          ...
          >
            ...
            {/* Dev Mode Firebase Health Diagnostic Button */}
            {import.meta.env.DEV && (
              <div className="mt-4 pt-3 border-t border-white/5 text-center">
                <button
                  type="button"
                  onClick={() => setShowFirebaseHealth(true)}
                  className="text-[11px] text-cyan-400/80 hover:text-cyan-300 font-mono inline-flex items-center gap-1.5 transition-colors cursor-pointer"
                >
        ...
        {showFirebaseHealth && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div
              ...
              onClick={() => setShowFirebaseHealth(false)}
            />
            <motion.div
            ...
            >
              <FirebaseHealthCheckPanel onClose={() => setShowFirebaseHealth(false)} />
            </motion.div>
          </div>
        )}
      </AnimatePresence>
