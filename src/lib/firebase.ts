...
//    Only initializes if the config is valid. If invalid, auth/db exports
//    will still exist but operations will fail with clear errors.
// ---------------------------------------------------------------------------
let app: FirebaseApp;
...
if (firebaseConfigValid) {
  ...
  // Initialize with the (broken) config anyway so that auth/db references
  // exist and error at operation-time rather than import-time. This prevents
  // the entire app from crashing on import and lets the UI show the config error.
  try {
    app = !getApps().length ? initializeApp(firebaseConfig) : getApp();
  } catch {
    // If initializeApp itself throws (e.g. completely empty apiKey), create
    // a minimal app so that getAuth/getFirestore don't crash on import.
    app = !getApps().length
...
//    Analytics failure must NEVER block authentication.
// ---------------------------------------------------------------------------
...
// Fire-and-forget analytics init — errors are caught and logged
initAnalytics();
